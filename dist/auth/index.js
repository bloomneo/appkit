...
import { getSmartDefaults, validateAuthConfig } from './defaults.js';
...
function get(overrides = {}) {
    if (globalAuthentication) {
        if (overrides && Object.keys(overrides).length > 0) {
            throw new Error(`[@bloomneo/appkit/auth] authClass.get() overrides are only applied on first call. Use authClass.reset(newConfig) to rebuild with new config. See: ${DOCS_URL}#configuration`);
        }
