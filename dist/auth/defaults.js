...
function parseRoleHierarchy() {
    ...
    for (const rolePair of rolePairs) {
        ...
        if (!roleLevel || !levelStr) {
            throw new Error(`[@bloomneo/appkit/auth] Invalid BLOOM_AUTH_ROLES entry: "${rolePair}". Expected "role.level:number". See: ${DOCS_URL}#role-level-permission-architecture`);
        }
        if (!validateRoleLevelFormat(roleLevel)) {
            throw new Error(`[@bloomneo/appkit/auth] Invalid role.level format: "${roleLevel}". Must be "role.level" (e.g., "admin.tenant"). See: ${DOCS_URL}#role-level-permission-architecture`);
        }
        const level = parseInt(levelStr);
        if (isNaN(level) || level < 1) {
            throw new Error(`[@bloomneo/appkit/auth] Invalid level number: "${levelStr}". Must be a positive integer. See: ${DOCS_URL}#role-level-permission-architecture`);
        }
...
function parseDefaultPermissions() {
    ...
    for (const permissionPair of permissionPairs) {
        ...
        if (parts.length !== 3) {
            throw new Error(`[@bloomneo/appkit/auth] Invalid BLOOM_AUTH_PERMISSIONS entry: "${permissionPair}". Expected "role.level:action:scope". See: ${DOCS_URL}#role-level-permission-architecture`);
        }
        ...
        if (!validatePermissionFormat(permission)) {
            throw new Error(`[@bloomneo/appkit/auth] Invalid permission format: "${permission}". Must be "action:scope" (e.g., "manage:tenant"). See: ${DOCS_URL}#role-level-permission-architecture`);
        }
...
export function validateSecret(secret) {
    if (!secret || typeof secret !== 'string') {
        throw new Error(`[@bloomneo/appkit/auth] BLOOM_AUTH_SECRET is required. Set a 32+ character random string. See: ${DOCS_URL}#configuration`);
    }
    if (secret.length < 32) {
        throw new Error(`[@bloomneo/appkit/auth] BLOOM_AUTH_SECRET must be at least 32 characters (got ${secret.length}). See: ${DOCS_URL}#configuration`);
    }
    if (secret === 'your-jwt-secret-key' || secret === 'secret' || secret === 'supersecret') {
        throw new Error(`[@bloomneo/appkit/auth] BLOOM_AUTH_SECRET looks like a default/example value. Use a strong random string. See: ${DOCS_URL}#configuration`);
    }
}
/**
...
function validateEnvironment() {
    ...
    if (rounds) {
        ...
        if (isNaN(roundsNum)) {
            throw new Error(`[@bloomneo/appkit/auth] Invalid BLOOM_AUTH_BCRYPT_ROUNDS: "${rounds}". Must be a number between 8 and 15. See: ${DOCS_URL}#configuration`);
        }
        validateRounds(roundsNum);
    }
    const expiresIn = process.env.BLOOM_AUTH_EXPIRES_IN;
    if (expiresIn && !isValidTimespan(expiresIn)) {
        throw new Error(`[@bloomneo/appkit/auth] Invalid BLOOM_AUTH_EXPIRES_IN: "${expiresIn}". Must be a valid time span (e.g. '7d', '1h', '30m'). See: ${DOCS_URL}#configuration`);
    }
    const nodeEnv = process.env.NODE_ENV;
    if (nodeEnv && !['development', 'production', 'test'].includes(nodeEnv)) {
        console.warn(`[@bloomneo/appkit/auth] Unusual NODE_ENV: "${nodeEnv}". Expected: development, production, or test.`);
    }
}
/**
...
export function validateAuthConfig(config) {
    if (!config?.jwt?.secret) {
        throw new Error(`[@bloomneo/appkit/auth] JWT secret required. Set BLOOM_AUTH_SECRET or pass jwt.secret in overrides. See: ${DOCS_URL}#configuration`);
    }
    validateSecret(config.jwt.secret);
    if (typeof config.password?.saltRounds !== 'number' || isNaN(config.password.saltRounds)) {
        throw new Error(`[@bloomneo/appkit/auth] password.saltRounds must be a number between 8 and 15. See: ${DOCS_URL}#configuration`);
    }
    validateRounds(config.password.saltRounds);
    if (config.jwt.expiresIn && !isValidTimespan(config.jwt.expiresIn)) {
        throw new Error(`[@bloomneo/appkit/auth] Invalid jwt.expiresIn: "${config.jwt.expiresIn}". Must be a valid time span (e.g. '7d', '1h', '30m'). See: ${DOCS_URL}#configuration`);
    }
}
/**
