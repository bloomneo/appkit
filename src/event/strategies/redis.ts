 ...
 */
export class RedisStrategy implements EventStrategy {
  ...
  once(event: string, handler: EventHandler): void {
    try {
      ...
      const onceWrapper: EventHandler = (data: any) => {
   ...
   */
  private buildChannelName(event: string): string {
   ...
   */
  private deserializeMessage(message: string): any {
    try {
   ...
   */
  private async unsubscribeFromEvent(event: string): Promise<void> {
    if (!this.connected || !this.subscriber) return;
   ...
   */
  private handleRedisMessage(event: string, message: string): void {
    try {
      // Deserialize message
